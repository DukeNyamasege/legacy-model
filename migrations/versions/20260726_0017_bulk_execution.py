...
def upgrade() -> None:
    ...
    op.create_index("ix_bulk_batches_signal", "bulk_execution_batches", ["signal_id"])
    op.create_index("ix_bulk_batches_run_status", "bulk_execution_batches", ["run_id", "status"])
    op.create_table(
    ...
    op.create_index("ix_bulk_members_batch", "bulk_execution_members", ["batch_id"])
    op.create_index("ix_bulk_members_account", "bulk_execution_members", ["managed_account_id"])
    with op.batch_alter_table("trades") as batch_op:
        ...
        batch_op.create_foreign_key(
            "fk_trades_managed_account_id",
            "managed_accounts",
            ["managed_account_id"],
            ["id"],
        )
        batch_op.create_foreign_key(
            "fk_trades_bulk_batch_id",
            "bulk_execution_batches",
            ["bulk_batch_id"],
            ["id"],
        )
    op.create_index("ix_trades_managed_account_id", "trades", ["managed_account_id"])
    op.create_index("ix_trades_bulk_batch_id", "trades", ["bulk_batch_id"])
