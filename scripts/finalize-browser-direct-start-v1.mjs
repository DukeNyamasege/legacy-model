...
for (const required of [
  ...
  if (!shell.includes(required)) throw new Error(`browser-direct-start-v1 canonical manual Start invariant missing: ${required}`);
}
if (shell.includes('await startBrowserDirectStrategy(snapshot.strategy);')) {
  throw new Error("browser-direct-start-v1 Builder still owns a separate manual Start path");
}
...
console.log("BROWSER_DIRECT_START_V1_INSTALLED resume_server=false custom_strategy_start_write=false run_builder_start_identical=true current_engine_strategy_fallback=true pending_map_close_fixed=true auto_trading_nonterminal_on_transport_error=true");
