...
class AIDRVirtualSoftGateTests(unittest.TestCase):
    def test_over4_uses_only_five_percent_tightening(self) -> None:
        self.assertAlmostEqual(POST_VIRTUAL_TIGHTENING_FACTOR, 1.05)
        self.assertAlmostEqual(POST_VIRTUAL_ALIGNMENT, 0.525)
        self.assertAlmostEqual(soft_alignment_for_barrier(4, 0.60), 0.525)

    def test_over4_uses_small_positive_live_edge(self) -> None:
    ...
    def test_soft_gate_never_tightens_a_lower_operator_threshold(self) -> None:
        self.assertAlmostEqual(soft_alignment_for_barrier(4, 0.50), 0.50)
        self.assertAlmostEqual(
