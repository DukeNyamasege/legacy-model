...
class HybridBrowserDirectV2Contract(unittest.TestCase):
    ...
    def test_run_panel_v6_is_start_stop_only_and_has_no_status_strips(self) -> None:
        ...
        self.assertIn("#11936f", authority)
        self.assertIn("#d13f4d", authority)
    ...
    def test_responsive_run_panel_totals_and_reopen_handle_cannot_be_hidden(self) -> None:
        ...
        self.assertIn('html[data-run-panel-visibility="open"] .app-main', usability)
        self.assertIn("padding-right:clamp(320px,25vw,460px)!important", usability)
    ...
    def test_builder_uses_one_connected_block_workspace(self) -> None:
        ...
        self.assertIn("color-mix(in srgb,var(--builder-block-accent) 16%,#07111f)", usability)
        self.assertIn("color-mix(in srgb,var(--builder-block-accent) 24%,#0a1728)", usability)

    def test_builder_draft_persists_and_market_dropdown_stays_open_until_user_closes(self) -> None:
        persistence = self.text("dashboard/direct-strategy-persistence-v1.js")
        self.assertIn('BUILDER_DRAFT_KEY = "derivadmin-builder-draft-v2"', persistence)
        self.assertIn('MARKET_OPEN_KEY = "derivadmin-builder-market-open-v1"', persistence)
        self.assertIn("window.FOA_FINAL_UI?.state?.()", persistence)
        self.assertIn("state.selectedStrategy = saved.selectedStrategy", persistence)
        self.assertIn("persistBuilderState", persistence)
        self.assertIn("bindMarketDropdown", persistence)
        self.assertIn('target.matches?.("[data-builder-market-mode-select]")', persistence)
        self.assertIn("saveMarketOpen(true)", persistence)
        self.assertIn('details.addEventListener("toggle"', persistence)
        self.assertNotIn("derivadmin:direct-reset-all", persistence)
    ...
    def test_transactions_have_time_market_type_exit_digit_buy_and_profit_columns(self) -> None:
        finalizer = self.text("scripts/finalize-production-controls-v6.mjs")
        ledger = self.text("dashboard/direct-transaction-ledger-v6.js")
        exporter = self.text("scripts/finalize-production-controls-v6b.mjs")
        self.assertIn("Time / Market", finalizer)
        self.assertIn("Exit digit", finalizer)
        self.assertNotIn("Entry / Exit", finalizer)
