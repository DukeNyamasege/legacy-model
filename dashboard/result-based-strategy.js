(() => {
  ...
  function clonePrimaryRoute() {
    const mode = selectedStrategyMode();
    const tradeType = String(builderValue("trade.side", "over"));
    const lastOperator = String(builderValue("lastRule.operator", ">="));
    const percentageTarget = String(builderValue("percentageRule.target", "even"));
    return {
      tradeType,
  ...
  function routeFromServer(route) {
    ...
    return {
      ...
      prediction: whole(route.prediction ?? 2, 2, 0, 9),
      durationTicks: whole(route.duration_ticks ?? 1, 1, 1, 100),
      analysisMode: mode,
      lastRule: {
        window: whole(digit?.window ?? 5, 5, 1, 1000),
        operator: String(lastOperator),
        value: whole(digit?.value ?? 3, 3, 0, 9),
      },
      percentageRule: {
        target: String(percentage?.target || "even"),
        value: whole(percentage?.value ?? 5, 5, 0, 9),
        window: whole(percentage?.window ?? 500, 500, 1, 1000),
        operator: String(percentage?.operator || ">="),
        threshold: clamp(n(percentage?.threshold, 70), 0, 100),
      },
      tickDirectionRule: {
        enabled: Boolean(direction),
        window: whole(direction?.window ?? 3, 3, 1, 1000),
        direction: String(direction?.direction || "rising"),
      },
    };
  }
  ...
  function routeConditions(route) {
    ...
    if (route.analysisMode === "last_digit" || route.analysisMode === "combined") {
      ...
      if (operator === "all_even" || operator === "all_odd") {
        conditions.push({
          kind: "digit_compare",
        ...
        conditions.push({
          ...
          window: whole(route.lastRule.window, 5, 1, 1000),
          operator,
          value: whole(route.lastRule.value, 3, 0, 9),
        });
      }
    }
    if (route.analysisMode === "percentage" || route.analysisMode === "combined") {
      ...
      if (["over", "under", "digit"].includes(percentage.target)) {
        percentage.value = whole(route.percentageRule.value, 5, 0, 9);
      }
    ...
    if (!conditions.length) {
      conditions.push({ kind: "digit_compare", window: 1, operator: ">=", value: 0 });
    }
    return conditions;
  }
  ...
  function resultRoutingPayload() {
    ...
    const route = state.afterLoss || clonePrimaryRoute();
    const tradeType = String(route.tradeType || "over");
    return {
      enabled: true,
      after_loss: {
        trade_type: tradeType,
        prediction: ["over", "under", "matches", "differs"].includes(tradeType)
          ? whole(route.prediction, 2, 0, 9)
          : null,
        duration_ticks: whole(route.durationTicks, 1, 1, 100),
  ...
  function hydrateServerPayload(payload) {
    if (!payload || typeof payload !== "object") return;
    if (payload.result_routing && typeof payload.result_routing === "object") {
    ...
    if (payload.martingale && typeof payload.martingale === "object") {
  ...
  function routePreview(route) {
    ...
    const prediction = ["over", "under", "matches", "differs"].includes(route.tradeType)
      ? ` ${whole(route.prediction, 2, 0, 9)}`
      : "";
  ...
  function routeSectionHtml() {
    ...
    return `<section class="builder-section result-routing-section" id="result-routing-section">
      ...
      <label class="result-routing-toggle">
        <span><strong>Use a different strategy after a loss</strong><small>Optional. Existing single-strategy trading stays unchanged while this is OFF.</small></span>
        <input id="result-routing-enabled" type="checkbox" ${state.routingEnabled ? "checked" : ""}>
      </label>
      <div class="result-routing-primary-note"><span><strong><b>WIN / First trade:</b> Primary strategy above</strong><small>The current Custom Strategy remains the normal route and is used again once recovery debt is fully cleared.</small></span></div>
      <div class="result-routing-recovery-box" ${state.routingEnabled ? "" : "hidden"}>
        <div class="result-routing-recovery-head"><span><strong>LOSS / Recovery Strategy</strong><small>Independent contract and independent analysis. Markets remain the same account markets selected above.</small></span><b class="result-routing-badge">AFTER LOSS</b></div>
        <div class="result-routing-grid">
          ...
          <label class="result-routing-field result-route-value-field" ${predictionHidden ? "hidden" : ""}>Prediction<input data-result-route="prediction" type="number" min="0" max="9" step="1" value="${whole(route.prediction, 2, 0, 9)}"></label>
          <label class="result-routing-field">Ticks<input data-result-route="durationTicks" type="number" min="1" max="100" step="1" value="${whole(route.durationTicks, 1, 1, 100)}"></label>
        ...
        <div class="result-routing-subsection result-routing-fields" ${showLast ? "" : "hidden"} data-result-block="last">
          ...
          <div class="result-routing-analysis-grid">
            <label class="result-routing-field">Check last<input data-result-route="lastRule.window" type="number" min="1" max="1000" step="1" value="${whole(route.lastRule.window, 5, 1, 1000)}"></label>
            ...
            <label class="result-routing-field result-route-value-field" ${lastValueHidden ? "hidden" : ""}>Value<input data-result-route="lastRule.value" type="number" min="0" max="9" step="1" value="${whole(route.lastRule.value, 3, 0, 9)}"></label>
          </div>
        </div>
        <div class="result-routing-subsection result-routing-fields" ${showPercentage ? "" : "hidden"} data-result-block="percentage">
          ...
          <div class="result-routing-analysis-grid">
            ...
            <label class="result-routing-field result-route-percentage-value" ${percentageValueHidden ? "hidden" : ""}>Digit<input data-result-route="percentageRule.value" type="number" min="0" max="9" step="1" value="${whole(route.percentageRule.value, 5, 0, 9)}"></label>
            <label class="result-routing-field">Past ticks<input data-result-route="percentageRule.window" type="number" min="1" max="1000" step="1" value="${whole(route.percentageRule.window, 500, 1, 1000)}"></label>
            ...
            <label class="result-routing-field">Threshold (%)<input data-result-route="percentageRule.threshold" type="number" min="0" max="100" step="0.1" value="${clamp(n(route.percentageRule.threshold, 70), 0, 100)}"></label>
          </div>
        </div>
        <div class="result-routing-subsection">
          ...
          <div class="result-routing-analysis-grid">
            ...
            <label class="result-routing-field">Check last<input data-result-route="tickDirectionRule.window" type="number" min="1" max="1000" step="1" value="${whole(route.tickDirectionRule.window, 3, 1, 1000)}"></label>
  ...
  function recoveryControlHtml() {
    return `<div class="recovery-spread-control" id="recovery-spread-control">
      <span><strong>Recovery plan</strong><small>Keep the current multiplier, or spread exact loss recovery across 1–3 successful recovery trades.</small></span>
      <div class="recovery-spread-grid">
        <label class="result-routing-field">Recovery style<select id="recovery-style"><option value="multiplier" ${state.recoveryMode === "multiplier" ? "selected" : ""}>Current multiplier — unchanged</option><option value="split" ${state.recoveryMode === "split" ? "selected" : ""}>Martingale Spread — exact debt</option></select></label>
        <label class="result-routing-field recovery-spread-parts" ${state.recoveryMode === "split" ? "" : "hidden"}>Successful recovery parts<select id="recovery-split-count">${[1, 2, 3].map((value) => `<option value="${value}" ${state.splitCount === value ? "selected" : ""}>${value} ${value === 1 ? "part" : "parts"}</option>`).join("")}</select></label>
      </div>
      <p class="recovery-spread-note">${state.recoveryMode === "split" ? `Spread mode: actual recovery debt is divided across ${state.splitCount} successful part${state.splitCount === 1 ? "" : "s"}. A losing recovery does not consume a part; actual settled profit reduces the remaining debt.` : "Multiplier mode remains exactly as configured in the Martingale field above. Spread is optional and does not affect existing users."}</p>
    </div>`;
  }
  ...
  function enhance() {
    ...
    if (accountKey !== state.accountKey) {
      ...
      state.recoveryTouched = false;
      window.setTimeout(loadServerState, 120);
    }
    ...
    window.fetch = async (input, init = {}) => {
      ...
      if (isCustom && method === "POST" && typeof init.body === "string") {
        try {
          ...
          if (state.serverHydrated || state.routingTouched) {
            body.result_routing = resultRoutingPayload();
          }
          if (state.recoveryHydrated || state.recoveryTouched) {
      ...
      if (isCustom && response.ok) {
        response.clone().json().then(hydrateServerPayload).catch(() => {});
      }
      return response;
    };
  }
  ...
  async function loadServerState() {
    ...
    try {
      ...
      if (response.ok) hydrateServerPayload(await response.json());
    } catch (_) {
      // Existing builder remains fully usable when this optional hydration read is delayed.
    } finally {
  ...
  document.readyState === "loading"
    ? document.addEventListener("DOMContentLoaded", () => {
        scheduleEnhance();
        window.setTimeout(loadServerState, 500);
      }, { once: true })
    : (() => {
        scheduleEnhance();
        window.setTimeout(loadServerState, 500);
  ...
  window.FOA_RESULT_BASED_STRATEGY_VERSION = "20260813-1";
})();
