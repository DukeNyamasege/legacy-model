(() => {
  ...
  function strategyView() {
    return `<section class="foa-page-intro"><div><span class="foa-eyebrow">ACTIVE AUTOMATION MODEL</span><h1>${esc(strategyName())}</h1><p>OVER 1 normal entries with controlled OVER 3 recovery.</p></div><span class="foa-online-pill">● System online</span></section><section class="foa-strategy-flow">${[["1", "Normal Mode", "DIGITOVER 1", "Used while there is no debt."], ["2", "First Recovery", "DIGITOVER 3", "Triggered after one real loss."], ["3", "Virtual Protection", "1 virtual win", "Used after the OVER 3 recovery loses."], ["4", "Full Recovery", "1 real DIGITOVER 4", "One winning contract targets all recorded debt."]].map(step => `<article class="foa-card"><span class="foa-step">${step[0]}</span><h2>${step[1]}</h2><strong>${step[2]}</strong><p>${step[3]}</p></article>`).join("")}</section><section class="foa-two-col"><article class="foa-card"><h2>Risk rule</h2><div class="foa-simple-list"><div><span>Virtual wins required</span><strong>1</strong></div><div><span>Recovery family</span><strong>OVER only</strong></div><div><span>PUT contracts</span><strong>Disabled</strong></div></div></article><article class="foa-card"><h2>Supported markets</h2><p>The worker scans configured synthetic markets and chooses only qualifying digit opportunities.</p><div class="foa-market-tags">${["1HZ10V", "1HZ25V", "1HZ50V", "1HZ75V", "1HZ100V", "R_10", "R_25", "R_50", "R_75", "R_100"].map(market => `<span>${market}</span>`).join("")}</div></article></section>`;
  }
