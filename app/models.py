...
class Trade(Base):
    ...
    entry_spot_display: Mapped[str | None] = mapped_column(String(100))
    exit_spot_display: Mapped[str | None] = mapped_column(String(100))
    entry_digit: Mapped[int | None] = mapped_column(Integer)
...
class DashboardSnapshot(Base):
    """Durable last-good aggregate dashboard payload (never personal data)."""

    __tablename__ = "dashboard_snapshots"

    account_type: Mapped[str] = mapped_column(String(10), primary_key=True)
    payload: Mapped[dict] = mapped_column(JSON, default=dict)
    generated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True))
    snapshot_version: Mapped[int] = mapped_column(Integer, default=1)
    source_watermark: Mapped[dict] = mapped_column(JSON, default=dict)
