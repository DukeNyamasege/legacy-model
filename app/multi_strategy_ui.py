...
_INSTALLED = False
UI_VERSION = "20260804-multi-strategy-2"
...
(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .foa-strategy-family b{display:block;font-size:14px;margin-bottom:5px}.foa-strategy-family span{display:block;color:var(--muted);font-size:11px;line-height:1.45}
      ...
      .foa-strategy-side-row{display:flex;gap:8px;flex-wrap:wrap;margin:8px 0 14px}.foa-strategy-side{appearance:none;min-width:112px;min-height:40px;padding:0 14px;border:1px solid var(--line);border-radius:10px;background:transparent;color:var(--text);font-weight:800;cursor:pointer}
      ...
      .foa-strategy-save-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.foa-strategy-save-row p{margin:0;color:var(--muted);font-size:10px;line-height:1.45}
      ...
      @media(max-width:760px){.foa-strategy-family-grid{grid-template-columns:1fr;gap:7px}.foa-strategy-family{min-height:76px;padding:10px;border-radius:11px}.foa-strategy-family b{font-size:11px}.foa-strategy-family span{font-size:9px}.foa-strategy-family em{font-size:7.5px;margin-top:6px}.foa-strategy-summary{grid-template-columns:1fr 1fr;padding:9px;gap:7px}.foa-strategy-summary span{font-size:7.5px}.foa-strategy-summary strong{font-size:9px}.foa-strategy-side{min-width:88px;min-height:34px;padding:0 10px;font-size:9px}.foa-strategy-save-row{align-items:stretch;flex-direction:column}.foa-strategy-save{width:100%;min-height:35px;font-size:9px}.foa-active-strategy-badge{font-size:8px;padding:5px 7px}}
    `;
  ...
  function familyMeta(family) { return payload?.catalog?.families?.[family] || null; }
  function isStopped() { return String(lifecycle?.lifecycle || "").toLowerCase() === "stopped"; }
  function renderKey() {
    const selection = currentSelection();
    return `${selection.family}:${selection.side}:${isStopped() ? "stopped" : "locked"}`;
  }
  ...
  function selectorMarkup() {
    ...
    return `
      <article class="foa-card foa-strategy-selector" id="foa-strategy-selector" data-render-key="${esc(key)}">
        ...
        <div class="foa-strategy-family-grid">${familyButtons}</div><div class="foa-strategy-side-row">${sideButtons}</div>
        <div class="foa-strategy-summary"><div><span>Selected</span><strong>${esc(selection.label)}</strong></div><div><span>Contract</span><strong>${esc(selection.contract_type)}</strong></div><div><span>Normal rule</span><strong>${esc(selection.normal_rule || "Validated qualifying signal")}</strong></div><div><span>Recovery</span><strong>${esc(selection.recovery_rule || "Same strategy family")}</strong></div></div>
  ...
  function ensureSelector() {
    ...
    if (!card) {
      grid.insertAdjacentHTML("afterbegin", selectorMarkup());
      card = document.getElementById("foa-strategy-selector");
    } else if (card.dataset.renderKey !== key && !card.contains(document.activeElement)) {
      card.outerHTML = selectorMarkup();
      card = document.getElementById("foa-strategy-selector");
    }
    bindSelector(card);
  }
  ...
  function setDraft(family, side) {
    ...
    payload.selection = { ...payload.selection, family: normalizedFamily, side: normalizedSide, family_label: meta.label, label: `${meta.label} · ${sideMeta.label}`, contract_type: sideMeta.contract_type, description: meta.description, normal_rule: sideMeta.normal_rule, recovery_rule: sideMeta.recovery_rule };
    dirty = true;
    const card = document.getElementById("foa-strategy-selector");
    if (card) card.outerHTML = selectorMarkup();
    bindSelector(document.getElementById("foa-strategy-selector"));
  ...
  function bindSelector(card) {
    ...
    card.querySelectorAll("[data-strategy-family]").forEach(button => { button.onclick = () => setDraft(button.dataset.strategyFamily, null); });
    card.querySelectorAll("[data-strategy-side]").forEach(button => { button.onclick = () => setDraft(currentSelection().family, button.dataset.strategySide); });
  ...
  async function saveStrategy() {
    ...
    try {
      const selection = currentSelection();
      const result = await jsonRequest("/me/strategy-settings", { method: "POST", body: JSON.stringify({ family: selection.family, side: selection.side }) });
  ...
  function ensureBadge() {
    ...
    if (!intro) return;
    const key = `${payload.selection.family}:${payload.selection.side}:${payload.selection.contract_type}`;
    let badge = intro.querySelector(".foa-active-strategy-badge");
    if (!badge) { badge = document.createElement("span"); badge.className = "foa-active-strategy-badge"; intro.appendChild(badge); }
    if (badge.dataset.strategyKey !== key) {
      badge.dataset.strategyKey = key;
      badge.innerHTML = `<i></i>${esc(payload.selection.label)} · ${esc(payload.selection.contract_type)}`;
    }
  }

  function strategyFlow(selection) {
    if (selection.family === "digits" && selection.side === "over") return [["1","Normal Mode","DIGITOVER 1","Current AIDR normal entry."],["2","First Recovery","DIGITOVER 3","Targets the first recorded loss."],["3","Virtual Protection","Virtual OVER 4","$0 confirmation after the loss trigger."],["4","Full Recovery","Real DIGITOVER 4","Uses live payout economics to target recorded debt."]];
    if (selection.family === "digits" && selection.side === "under") return [["1","Normal Mode","DIGITUNDER 8","Mirrored high-probability Under entry."],["2","First Recovery","DIGITUNDER 6","Mirrored first-recovery condition."],["3","Virtual Protection","Virtual UNDER 5","$0 confirmation using the soft 52.5% gate."],["4","Full Recovery","Real DIGITUNDER 5","Uses the same account debt and live payout sizing."]];
    if (selection.family === "parity") return [["1","Parity Scan",selection.contract_type,"20/50/100/500-window parity alignment."],["2","Proposal Check","Live break-even","Requires a positive edge above the current quote."],["3","Virtual Protection",`Virtual ${selection.side.toUpperCase()}`,"$0 observation while the account is protected."],["4","Recovery",`Real ${selection.side.toUpperCase()}`,"The same parity family uses live payout-based debt sizing."]];
    return [["1","RF-DIR5 Scan",selection.side.toUpperCase(),"Five-move direction, impulse and exhaustion filters."],["2","Proposal Check",selection.contract_type,"One-tick CALL/PUT quote must retain a positive edge."],["3","Virtual Protection",`Virtual ${selection.side.toUpperCase()}`,"$0 directional observation after the loss trigger."],["4","Recovery",`Real ${selection.contract_type}`,"The same direction family recovers from live proposal economics."]];
  }
  ...
  function applyStrategyPresentation() {
    ...
    document.querySelectorAll('[data-control="start"]').forEach(button => {
      const text = `Start ${label} AutoTrade`;
      if (!button.querySelector("svg") && button.textContent !== text) button.textContent = text;
    ...
    const key = `${payload.selection.family}:${payload.selection.side}`;
    if (flow && intro?.classList?.contains("foa-page-intro") && flow.dataset.strategyKey !== key) {
      flow.dataset.strategyKey = key;
      const h1 = intro.querySelector("h1"); const p = intro.querySelector("p");
  ...
  async function load(force = false) {
    if (loading || (dirty && !force)) return;
    const now = Date.now();
    if (!force && now - lastLoad < 7000) return;
    loading = true;
    try {
      const [strategyPayload, lifecyclePayload] = await Promise.all([jsonRequest("/me/strategy-settings"), jsonRequest("/me/trading-lifecycle")]);
      payload = strategyPayload; lifecycle = lifecyclePayload; lastLoad = Date.now();
      ensureSelector(); applyStrategyPresentation();
    } catch (_) {
      // Logged-out public dashboard: strategy selection is personal only.
    } finally { loading = false; }
  }

  function apply() { ensureStyles(); ensureSelector(); applyStrategyPresentation(); load(false); }
  const observer = new MutationObserver(() => apply());
  function start() { observer.observe(document.documentElement, { childList: true, subtree: true }); load(true); apply(); window.setInterval(apply, 2500); }
...
def _append(source: str) -> str:
    if "FOA_MULTI_STRATEGY_UI_VERSION:20260804-2" not in source:
