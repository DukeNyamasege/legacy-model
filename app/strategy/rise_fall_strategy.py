...
def detect_rise_candidate(
    ...
    *,
    ...
    minimum_recent_directional_moves: int = 2,
    minimum_efficiency: float = 0.35,
) -> bool:
    recent_moves = features.movements[-minimum_recent_directional_moves:]
    return (
        features.up_count >= minimum_directional_moves
        and features.net_move > 0
        and len(recent_moves) == minimum_recent_directional_moves
        and all(move > 0 for move in recent_moves)
