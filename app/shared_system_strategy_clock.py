...
def _manual_groups(
    ...
    grouped: dict[tuple[str, str, int | None], tuple[Any, set[int]]] = {}
    for route in multi._strategy_snapshot(bot, force=True):
...
async def _shared_clock_buy_for_scope(
    ...
    *,
    ...
    role = _route_role(source)
    system_scope = _system_ids(bot, scope)
    manual_groups = _manual_groups(bot, scope)
    accounted = set(system_scope)
    for _selection, ids in manual_groups:
    ...
    prepared: list[tuple[Any, set[int], Any, float, str]] = []
    for selection, ids in manual_groups:
        ...
        prepared.append((selection, set(ids), clone, predicted, contract))

    proposal_results = await asyncio.gather(
        *(
            multi._proposal_for(bot, clone, predicted)
            for _selection, _ids, clone, predicted, _contract in prepared
        ),
    ...
    for prepared_item, result in zip(prepared, proposal_results, strict=True):
        selection, ids, clone, predicted, contract = prepared_item
        if isinstance(result, Exception):
            bot.repository.mark_signal(clone.signal_id, status="SKIP_SHARED_CLOCK_PROPOSAL_EXCEPTION")
            _notify_group_skip(
                bot,
                ids,
                selection=selection,
            ...
            bot.logger.exception(
                "SHARED_CLOCK_PROPOSAL_FAILED family=%s side=%s role=%s signal_id=%s",
                selection.family,
                selection.side,
                role,
                clone.signal_id,
            ...
            continue
        if result is None:
            bot.repository.mark_signal(clone.signal_id, status="SKIP_SHARED_CLOCK_INVALID_PROPOSAL")
            _notify_group_skip(
                bot,
                ids,
                selection=selection,
        ...
        bot.logger.warning(
            "SHARED_SYSTEM_CLOCK_ROUTE source_signal=%s routed_signal=%s role=%s "
            "family=%s side=%s symbol=%s contract_type=%s barrier=%s accounts=%s "
    ...
    bot.logger.warning(
        ...
        len(system_scope),
        len(manual_groups),
        sum(len(ids) for _selection, ids in manual_groups),
        len(unknown),
    )
